...
def labels_match_clusters(topics_df: pd.DataFrame, clusters_df: pd.DataFrame) -> bool:
    if not {"cluster_id", "cluster_size"}.issubset(topics_df.columns):
        return False

    expected_sizes = (
        clusters_df.loc[clusters_df["cluster"] != -1, "cluster"]
        .value_counts()
        .sort_index()
    )
    saved_sizes = (
        topics_df.set_index("cluster_id")["cluster_size"]
        .astype(int)
        .sort_index()
    )
    return saved_sizes.equals(expected_sizes)
