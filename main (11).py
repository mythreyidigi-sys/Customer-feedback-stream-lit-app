...
with pipeline_tab:
    ...
    st.caption(
        "Read-only checklist showing which stages of the offline data pipeline "
        "have been run, based on whether each stage's expected output file "
        "exists in this repo. These scripts (scraping, embedding generation, "
        "HDBSCAN clustering, etc.) run locally/offline -- they are not "
        "executed inside this deployed app, since they need heavier compute "
        "and raw data folders than a cloud dashboard should carry."
    )
