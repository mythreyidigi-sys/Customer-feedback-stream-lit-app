    ...
    if not SERVQUAL_MODULE_OK:
        ...
        if responses_df is None:
            ...
            if likert_cols:
                ...
                real_rows = responses_df.dropna(how="all")

            st.metric("Clean survey responses", len(real_rows))

            if likert_cols and len(real_rows) >= 2:
                scores = score_survey(real_rows)
                scores_sorted = scores.sort_values("mean_gap")
                ...
                st.info(
                    f"Loaded {len(real_rows)} response row(s) from '{servqual_source}', but the "
                    "columns don't match the expected 'Dimension|item|E/P' format used by "
                    "servqual_survey_analysis.py. Use the field-survey template's exact column "
                    "layout, or map your Google-Forms export to that format first."
                )
                st.dataframe(real_rows.head(20), use_container_width=True)
