...
# SERVQUAL survey analysis script: sits as a plain .py file alongside main.py.
try:
    from servqual_survey_analysis import (
        score_survey, triangulate, SERVQUAL_ITEMS,
        DIMENSION_TO_ISSUE_CATEGORY, INTERIM_ISSUE_FREQUENCY,
    )

    SERVQUAL_MODULE_AVAILABLE = True
    SERVQUAL_IMPORT_ERROR = None
except Exception as exc:  # noqa: BLE001
    SERVQUAL_MODULE_AVAILABLE = False
    SERVQUAL_IMPORT_ERROR = str(exc)
...
classify_tab, reviews_tab, anomaly_tab, priority_tab, clusters_tab, servqual_tab, template_tab, reputation_tab, emotion_tab, pipeline_tab, about_tab = st.tabs(
    [
...
with servqual_tab:
    ...
    if servqual_scores is None or servqual_triangulation is None:
        ...
        st.caption(
            ...
            "The analysis above uses the pre-computed servqual_dimension_scores.csv / "
            "servqual_nlp_triangulation.csv (i.e. whatever was last saved by running "
            "servqual_survey_analysis.py offline)."
        )

    st.subheader("Recompute Live from Filled-In Survey Responses")
    st.caption(
        "Reads real respondent rows from servqual_survey_template.xlsx's 'Raw "
        "Responses' sheet (if filled in) and recomputes gap scores + "
        "triangulation on the spot using servqual_survey_analysis.py's actual "
        "score_survey() and triangulate() functions -- not the static CSVs above."
    )
    if not SERVQUAL_MODULE_AVAILABLE:
        st.warning(
            "servqual_survey_analysis.py was not found next to main.py. "
            "Upload it to the repo root to enable live recomputation."
        )
        with st.expander("Import error details"):
            st.code(SERVQUAL_IMPORT_ERROR or "Unknown import error")
    else:
        template_path_live = BASE_DIR / "servqual_survey_template.xlsx"
        if not template_path_live.exists():
            st.info("servqual_survey_template.xlsx not found -- nothing to recompute yet.")
        else:
            try:
                raw_responses = pd.read_excel(template_path_live, sheet_name="Raw Responses")
            except Exception as exc:
                raw_responses = None
                st.warning(f"Could not read the 'Raw Responses' sheet: {exc}")

            if raw_responses is not None:
                # Real respondent rows = any row with at least one non-null
                # Likert value, beyond the single highlighted example row (R1).
                likert_cols = [c for c in raw_responses.columns if "|" in c]
                filled_mask = raw_responses[likert_cols].notna().any(axis=1)
                real_responses = raw_responses[filled_mask].reset_index(drop=True)

                st.metric("Filled-in respondent rows found", len(real_responses))

                if len(real_responses) < 2:
                    st.info(
                        "Fewer than 2 filled-in rows (beyond the built-in example). "
                        "Fill in the 'Raw Responses' sheet with real 10-15 "
                        "respondents and re-upload the workbook to see a live "
                        "recomputed result here."
                    )
                else:
                    live_scores = score_survey(real_responses)
                    st.dataframe(live_scores, hide_index=True, use_container_width=True)

                    # Prefer real NLP frequencies from classified reviews if
                    # available, else fall back to the script's interim table.
                    cx_issue_col_check = None
                    if classified_reviews is not None:
                        cx_issue_col_check = find_column(
                            classified_reviews,
                            ["predicted_issue_category", "issue_cluster", "issue_category"],
                        )
                    if cx_issue_col_check:
                        issue_frequency = classified_reviews[cx_issue_col_check].value_counts().to_dict()
                        st.caption("Using live issue-category frequencies from your classified reviews.")
                    else:
                        issue_frequency = INTERIM_ISSUE_FREQUENCY
                        st.caption("Using the script's built-in interim issue-frequency table (no classified reviews loaded).")

                    live_merged, live_rho = triangulate(live_scores, issue_frequency)
                    st.dataframe(live_merged, hide_index=True, use_container_width=True)
                    st.metric("Spearman rho (gap severity vs. NLP frequency)", f"{live_rho:.2f}")
...
with pipeline_tab:
    ...
    st.caption(
        "Read-only checklist for the full pipeline, merge_reviews.py through "
        "servqual_survey_analysis.py, showing which stages have their "
        "expected output file present in this repo. Stages 1-9 (scraping, "
        "embedding generation, HDBSCAN clustering, etc.) are heavy and "
        "run locally/offline only -- they are not executed inside this "
        "deployed app. Stages 10-14 (emotion, root-cause, escalation, "
        "reply drafting, SERVQUAL) are lightweight enough to also run "
        "live inside the app itself, via the 'Emotion & Escalation' and "
        "'SERVQUAL Survey' tabs -- this checklist just reflects whether "
        "their output files exist in the repo, regardless of how they "
        "were produced."
    )

    pipeline_stages = [
        ("1. Merge Reviews", "merge_reviews.py", "data/merged_reviews/all_restaurants_reviews.xlsx"),
        ("2. Clean Reviews", "clean_reviews.py", "outputs/cleaned_reviews.xlsx"),
        ("3. Generate Embeddings", "generate_embeddings.py", "models/embeddings.pkl"),
        ("4. Reduce Dimensions (UMAP)", "Reduced_dimention.py", "models/embeddings_reduced.pkl"),
        ...
        ("10. Emotion-Layer Classification", "emotion_classification.py", "reviews_with_emotion.csv"),
        ("11. Root Cause x Emotion Cross-Tab", "root_cause_emotion_analysis.py", "dominant_emotion_per_issue.csv"),
        ("12. Escalation / Urgency Detection", "escalation_detection.py", "escalations.csv"),
        ("13. Empathetic Reply Drafts", "empathetic_reply_generator.py", "reviews_with_replies.csv"),
        ("14. SERVQUAL Survey Analysis", "servqual_survey_analysis.py", "servqual_dimension_scores.csv"),
    ]

    rows = []
    for stage_name, script_name, expected_path in pipeline_stages:
        exists = (BASE_DIR / expected_path).exists()
        rows.append({
            "Stage": stage_name,
            "Script": script_name,
            "Expected Output": expected_path,
            "Status": "Complete" if exists else "Not yet run",
        })
    status_df = pd.DataFrame(rows)

    completed = (status_df["Status"] == "Complete").sum()
    st.metric("Pipeline stages complete", f"{completed} / {len(status_df)}")
    st.progress(completed / len(status_df))

    st.caption(
        "Note: cx_common.py is a shared helper required by stages 10-13 "
        "(emotion, root-cause, escalation, reply drafting) -- it has no "
        "output file of its own, so it isn't listed as a separate stage, "
        "but must be present alongside main.py for those stages to run."
    )

    def _highlight_status(row):
        color = "background-color: #d4edda" if row["Status"] == "Complete" else "background-color: #fff3cd"
        return [color] * len(row)

    st.dataframe(
        status_df.style.apply(_highlight_status, axis=1),
        hide_index=True, use_container_width=True,
    )

    st.caption(
        "To advance a stage, run its script locally (or in Colab/VS Code), then "
        "commit the resulting output file to this repo at the path shown above -- "
        "the next time this app reboots, that stage will show as Complete and "
        "the corresponding dashboard tab(s) will populate automatically."
    )
...
with about_tab:
    ...
    if not readme_path.exists():
        st.error(
            "README.md is required but was not found at the repo root "
            "(next to main.py). Please add a README.md file to enable this tab."
        )
        st.stop()

    # st.markdown() doesn't resolve local image paths in markdown syntax
    # (![alt](path)) -- it expects a URL. So split the README on any such
    # image lines and render local images with st.image() instead, keeping
    # everything else as normal markdown.
    readme_text = readme_path.read_text(encoding="utf-8")
    image_pattern = re.compile(r"!\[([^\]]*)\]\(([^)\s]+)\)")

    last_end = 0
    for match in image_pattern.finditer(readme_text):
        st.markdown(readme_text[last_end:match.start()])
        alt_text, image_ref = match.group(1), match.group(2)
        if not image_ref.startswith(("http://", "https://")):
            image_path = BASE_DIR / image_ref
            if image_path.exists():
                st.image(str(image_path), caption=alt_text or None, use_container_width=True)
            else:
                st.info(f"Referenced image not found in repo: {image_ref}")
        else:
            st.markdown(match.group(0))
        last_end = match.end()
    st.markdown(readme_text[last_end:])
