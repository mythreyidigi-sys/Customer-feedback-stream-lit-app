...
def load_data() -> pd.DataFrame:
    ...
    if "cluster" in df.columns and os.path.exists(LABEL_PATH):
        ...
        if {"cluster_id", "issue_label"}.issubset(labels.columns):
            df[ISSUE_CATEGORY_COLUMN] = df["cluster"].map(
                labels.set_index("cluster_id")["issue_label"]
            )
    ...
    df.attrs["rating_is_imputed"] = rating_is_imputed
    df.attrs["date_is_imputed"] = date_is_imputed
    return df
...
if df.attrs.get("rating_is_imputed") or df.attrs.get("date_is_imputed"):
    missing_fields = []
    if df.attrs.get("rating_is_imputed"):
        missing_fields.append("ratings")
    if df.attrs.get("date_is_imputed"):
